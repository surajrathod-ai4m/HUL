...
table_name = 'hul_4_jul_data'

# Define the CSV file path
csv_file_path = '/home/suraj-rathod/Desktop/HUL/Good cycle detection/csv/day_4.csv'

# Read the CSV file into a pandas DataFrame
df = pd.read_csv(csv_file_path)

# Function to create plc_timestamp column as string
def create_plc_timestamp_str(row):
    ...
    return dt.strftime('%Y-%m-%d %H:%M:%S.%f')  # Format datetime as string

# Add plc_timestamp column to DataFrame
df['plc_timestamp_str'] = df.apply(create_plc_timestamp_str, axis=1)
...
for index, row in df.iterrows():
    ...
    # Use the plc_timestamp_str directly
    plc_timestamp_str = row['plc_timestamp_str']
    ...
    cursor.execute(insert_query, {
        ...
        'sl21_seal_strength': row['sl21_seal_strength'], 'sl21_pulling_servo_motor_current': row['sl21_pulling_servo_motor_current'], 'spare1': row['machine_status_code'], 'spare2': row['horizontal_servo_position'], 'spare3': row['vertical_servo_position'], 'spare4': row['rotational_valve_position'],
        'spare5': row['fill_piston_position'], 'spare6': row['web_puller_position'], 'spare7': row['cam_position'], 'spare8': row['machine_status'], 'spare9': row['spare9'], 'spare10': row['spare10'],
        'spare11': row['spare11'], 'spare12': row['spare12'], 'spare13': row['spare13'], 'spare14': row['spare14'], 'spare15': 0 , 'cycle_time_ms': row['cycle_time_ms'], 'plc_timestamp': plc_timestamp_str
    })
